...
func (s *UserStore) Create(ctx context.Context, tx *sql.Tx, user *User) error {
  ...
  `
	...
	err := tx.QueryRowContext(ctx, query, user.Username, user.Email, user.Password.hash, user.RoleID, role).
...
func (s *UserStore) GetById(ctx context.Context, userId int64) (*User, error) {
  ...
  `
	...
	err := s.db.QueryRowContext(ctx, query, userId).
		Scan(&user.ID, &user.Username, &user.Email, &user.Password.hash, &user.CreatedAt, &user.IsActive, &user.Role.ID, &user.Role.Name, &user.Role.Level, &user.Role.Description)
	if err != nil {
		switch err {
		case sql.ErrNoRows:
...
func (s *UserStore) Activate(ctx context.Context, hashToken string) error {
	return withTx(s.db, ctx, func(tx *sql.Tx) error {
		// 1. find the user from user invitation token
		user, err := s.getUserFromInvitation(ctx, tx, hashToken)
		if err != nil {
			return err
		}

		// 2. update the user active status
		user.IsActive = true
		if err := s.update(ctx, tx, user); err != nil {
			return err
		}

		// 3. clean the user invitation
...
func (s *UserStore) getUserFromInvitation(ctx context.Context, tx *sql.Tx, hashToken string) (*User, error) {
	query := `
